...
from multiworld.envs.mujoco.cameras import sawyer_pick_and_place_camera, init_single_camera
...
class SawyerPickAndPlaceEnvV2(MultitaskEnv, SawyerXYZEnv):
    def __init__(
            self,
            obj_low=None,
            obj_high=None,
        ...
        # audrey 
        # print('p object in hand: ', self.p_obj_in_hand)
    ...
    def viewer_setup(self):
        ...
        #     init_single_camera(self.viewers[i].cam, i, self.num_cameras)

    def multi_viewer_setup(self, num_ele, num_azi, ele_low, ele_high, ele_offset, azi_low, azi_high, azi_offset):
        self.num_cameras =  num_ele * num_azi 
        list_ele = np.linspace(ele_low, ele_high, num=num_ele) + ele_offset
        list_azi = np.linspace(azi_low, azi_high, num=num_azi) + azi_offset
        ...
        for i in range(num_ele): 
            for j in range(num_azi): 
                sawyer_pick_and_place_camera(self.viewers[index].cam)
                self.viewers[index].cam.elevation = list_ele[i]
                self.viewers[index].cam.azimuth = list_azi[j]
                self.viewers[index].cam.trackbodyid = index
    ...
    def step(self, action):
        ...
        # gets rgb and depth arrays n_cameras x height x width
        rgb, depth = self.get_image(height=height, width=width)
    ...
    def _get_angles(self): 
        angles = np.zeros([self.num_cameras, 2])
        for i in range(self.num_cameras): 
            angles[i, 0] = self.viewers[i].cam.elevation
            angles[i, 1] = self.viewers[i].cam.azimuth
        return angles
    ...
    def reset_model(self):
        ...
        # if self.eval_pickups > 4000: 
        #     print('CHANGING TO 1.', self.eval_pickups)
        #     self.p_obj_in_hand = 1.
        # elif self.eval_pickups > 2000: 
        #     print('CHANGING TO .9', self.eval_pickups)
        #     self.p_obj_in_hand = 0.9
        return self._get_obs()
    ...
    def compute_rewards(self, actions, obs):
        ...
        if self.reward_type == 'hand_distance':
            ...
            r = -(
                hand_and_obj_distances > self.indicator_threshold # changed this
            ).astype(float)
        elif self.reward_type == 'touch_distance':
            r = -touch_distances
        elif self.reward_type == 'touch_success':
    ...
    def get_diagnostics(self, paths, prefix=''):
        ...
        print('TIME STEP ', statistics['timesteps_total'])
        if statistics['timesteps_total'] > 6000000: 
            self.p_obj_in_hand = 1.
        elif statistics['timesteps_total'] > 4000000: 
            self.p_obj_in_hand = 0.9
