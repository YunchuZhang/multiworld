...
class SawyerPushAndReachXYZRandObjectEnv(MultitaskEnv, SawyerXYZEnv):
    def __init__(
            self,
            puck_low=(-.4, .2),
            puck_high=(.4, 1),
    ...
    def change_model(self, model_path):
        if model_path.startswith("/"):
            ...
            fullpath = os.path.join(os.path.dirname(multiworld.__file__), "envs/assets", model_path)
        ...
        self.viewer = None
        if self.camera_init_fctn is not None:
            self.viewers = []
            self.initialize_camera(self.camera_init_fctn, self.num_cameras)
...
class SawyerPushAndReachXYRandObjectEnv(SawyerPushAndReachXYZRandObjectEnv):
    def __init__(self, *args, hand_z_position=0.05, **kwargs):
        self.quick_init(locals())
        SawyerPushAndReachXYZRandObjectEnv.__init__(self, *args, **kwargs)
