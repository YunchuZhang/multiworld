...
class Sawyerpenmugtask(SawyerXYZEnv, MultitaskEnv):
    def __init__(
            self,
    ...
    def _get_obs(self):
        ...
        flat_obs = np.concatenate((e-start_frame, b, c))
        return dict(
            observation=flat_obs,
            desired_goal=self._state_goal-start_frame,
            achieved_goal=e-start_frame
    ...
    @property
    def model_name(self):
        return get_asset_full_path('sawyer_xyz/sawyer_penmug.xml')
    ...
    def _sample_puck_xys(self):
        if self._always_start_on_same_side:
            return np.array([0.2, 0.7]), np.array([-0.1, 0.6])
        else:
    ...
    def _set_puck_xys(self, puck_xys):
        ...
        qpos[11:15] = np.array([0.7071068, 0, 0.7071068, 0,])
